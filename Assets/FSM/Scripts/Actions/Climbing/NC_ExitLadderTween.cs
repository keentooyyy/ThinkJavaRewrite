...
public class NC_ExitLadderTween : ActionTask<Transform>
{
    ...
    protected override void OnExecute()
    {
        ...
        if (sensor != null)
        {
            boundY = side == ExitSide.Top ? sensor.LadderTopY() + Mathf.Abs(placeOffset)
        ...
        {
            if (runSimultaneously) { seq.Append(yTween); seq.Join(xTween); }
            else { seq.Append(yTween); seq.Append(xTween); }
        }
        else
        {
            seq.Append(yTween);
